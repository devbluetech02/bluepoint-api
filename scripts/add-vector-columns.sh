#!/bin/bash
# Aplica a migration 007: coluna embedding vector(1536) em todas as tabelas do schema bluepoint.
...
VECTOR_PORT="5437"
VECTOR_USER="bluepoint"
VECTOR_DB="bluepoint_vector"
VECTOR_PASS="Bluetech*9090"

export PGPASSWORD="$VECTOR_PASS"

echo ">>> Aplicando migration: coluna embedding em todas as tabelas (schema bluepoint)..."
psql -h "$VECTOR_HOST" -p "$VECTOR_PORT" -U "$VECTOR_USER" -d "$VECTOR_DB" -v ON_ERROR_STOP=1 -f "$MIGRATION"

unset PGPASSWORD
echo ">>> Concluído. Todas as tabelas do schema bluepoint têm agora a coluna embedding vector(1536)."
